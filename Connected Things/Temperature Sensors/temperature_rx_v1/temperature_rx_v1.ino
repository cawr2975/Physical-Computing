...
void loop()
{
  ...
  if (rf_driver.recv(buf, &buflen))
  {
    ...
    if(str_out.substring(0,4) == "cgw1"){
      Serial.println("got msg1 ");
    }

    if(str_out.substring(0,4) == "cgw2"){
      Serial.println("got msg2 ");
    }
    
    if(str_out.substring(0,3) == "cgw"){
      Serial.print("got a sensor msg: ");
      Serial.println(str_out.substring(5, buflen));

      for(int i = 3; i <str_out.length(); i++){
      if(str_out.substring(i, i+1) == ","){
        str_analogVal = str_out.substring(4, i); // starting 3 characters in, after the message flag
